...
class UdpReceiverThread(QThread):
    ...
    def __init__(self, port_getter, parent=None):
...
class configPage(QMainWindow, Ui_MainWindow):
    ...
    def bindSingalandSlot(self):
        ...
        # Timer connections (connected once to avoid duplicate signal connections)
    ...
    def PrepareSpectrumIdata(self):
        self.IdataSpectrum = [
            (i if i <= 1024 else i - 2048) * 125 / 2048
            for i in range(2048)
        ]
...
if __name__ == "__main__":
    ...
    statisticThread = UdpReceiverThread(lambda: portStatistical)
    statisticThread.start()
    statisticThreadLDPC = UdpReceiverThread(lambda: LDPCportStatistical)
    statisticThreadLDPC.start()
    spectrumThread = UdpReceiverThread(lambda: SpectrumPortStatistical)
    spectrumThread.start()
    IQdataThread = UdpReceiverThread(lambda: portIQ)
    IQdataThread.start()
    SSSystemThread = UdpReceiverThread(lambda: portSSSys)
    SSSystemThread.start()
    CorrValueThread = UdpReceiverThread(lambda: portCorrValue)
