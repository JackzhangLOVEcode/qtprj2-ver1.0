...
def getChosenIP(segment):
    ...
    ipAddrInfoList = socket.getaddrinfo(hostName, None, socket.AF_INET)
    ...
    try:
        with open(portFile, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                line = line.strip()
                if line and i < len(defaults):
                    defaults[i] = int(line.split(":")[1])
