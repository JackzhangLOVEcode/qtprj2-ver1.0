...
class ConnectRFConfig():
    ...
    def _prepareConfig(self, fpgaindex:int, wrinfo:int, fmcinfo:int, registerinfo, value:int):
        """公共方法：设置基础信息、寄存器信息和数据值，返回配置数据"""
        # setBaseInfo
    ...
    def connectFreqInfo(self, fpgaindex:int, wrinfo:int, fmcinfo:int, registerinfo, value):
        ...
        return self._prepareConfig(fpgaindex, wrinfo, fmcinfo, registerinfo, int(value * 10 ** 9 / 100))

    def connectTXAttenuationInfo(self, fpgaindex:int, wrinfo:int, fmcinfo:int, registerinfo, channel1, channel2):
        """用于组织发射衰减配置数据"""
        return self._prepareConfig(fpgaindex, wrinfo, fmcinfo, registerinfo, int(channel1/0.5*2**8 + channel2/0.5))

    def connectRXGainInfo(self, fpgaindex:int, wrinfo:int, fmcinfo:int, registerinfo, agcenable, channel1, channel2, step):
        """用于组织接收增益、观察接收增益配置数据"""
        return self._prepareConfig(fpgaindex, wrinfo, fmcinfo, registerinfo, int(agcenable*2**16 + channel1/step*2**8 + channel2/step))
    ...
    def connectTXRXEnableInfo(self, fpgaindex:int, wrinfo:int, fmcinfo:int, registerinfo, rx1, rx2, tx1, tx2):
        """用于组织收发通道使能配置数据"""
        return self._prepareConfig(fpgaindex, wrinfo, fmcinfo, registerinfo, int(rx1*2**3 + rx2*2**2 + tx1*2**1 + tx2))
    ...
    def connectClockInfo(self, fpgaindex:int, wrinfo:int, fmcinfo:int, registerinfo, outerclock, sourceclock):
        """用于组织时钟选择配置数据"""
        return self._prepareConfig(fpgaindex, wrinfo, fmcinfo, registerinfo, int(outerclock*2**2 + sourceclock))
